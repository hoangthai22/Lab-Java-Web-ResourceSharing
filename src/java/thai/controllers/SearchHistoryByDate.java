 ...
 * @author ASUS
 */
@WebServlet(name = "SearchHistoryByDate", urlPatterns = {"/SearchHistoryByDate"})
public class SearchHistoryByDate extends HttpServlet {
    
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        try {
            String date = request.getParameter("txtRequestDate");
            HttpSession session = request.getSession();
            RegistrationObj user = (RegistrationObj) session.getAttribute("user");
            BookingDAO bookingDao = new BookingDAO();
            List<BookingObj> listBooking = bookingDao.searchBookingByRequestDate(user.getUserId(), date);
